...
pub async fn submit_certificate(
    ...
    if user.auth_type != "wallet" {
        return Err(ApiError::BadRequest("Certificators must use wallet login".into()));
    }
...
pub async fn list_pool_certificates(
    ...
    let certificates: Vec<Certificate> = if let Some(status) = status_filter {
        ...
        sqlx::query_as(
            "SELECT * FROM certificates WHERE pool_id = $1 ORDER BY created_at DESC"
        )
        .bind(pool.id)
        .fetch_all(&state.db)
        .await
...
pub async fn verify_certificate(
    ...
    let cert: Option<Certificate> = sqlx::query_as(
        "SELECT * FROM certificates WHERE document_hash = $1 AND status = 'minted'"
    )
    .bind(&hash)
    .fetch_optional(&state.db)
    .await
